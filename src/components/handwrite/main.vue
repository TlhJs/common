...
export default {
     ...
     */
        initCanvas() {
            ...
            const { boardCanvas } = this.$refs; // 获取canvas元素
     ...
     */
        onTouchStart(e) {
            const { offsetLeft } = e.target; // 获取canvas距离页面左边的距离
            const { offsetTop } = e.target; // 获取canvas距离页面顶部的距离
            const targetX = e.touches[0].clientX - offsetLeft;
            const targetY = e.touches[0].clientY - offsetTop;
            this.ctx.beginPath();
